...
// Layout pattern 7 foto per halaman — persis seperti screenshot
const LAYOUT_SPANS = [
  "row-span-2",              // 0: tall kiri
  "",                        // 1: normal
  "",                        // 2: normal
  "row-span-2",              // 3: tall kanan
  "col-span-2 row-span-2",   // 4: wide featured tengah
  "",                        // 5: bawah kiri
  "",                        // 6: bawah kanan
];
  ...
  return (
    <div className="min-h-screen bg-surface-100">
      ...
      <section className="max-w-[1200px] mx-auto px-4 sm:px-6 py-10">
        <h2 className="text-3xl font-bold text-ink-900 mb-8">Galeri Armada</h2>

        <div className="grid grid-cols-4 gap-4 auto-rows-[140px]">
          {pagePhotos.map((photo, i) => (
            <div
              key={`${page}-${i}`}
              className={`relative overflow-hidden rounded-2xl shadow-card ${LAYOUT_SPANS[i] ?? ""}`}
            >
              <Image
                src={photo.src}
                alt={photo.nama}
              ...
              {Array.from({ length: totalPages }).map((_, i) => (
                <button
                  ...
                  className={`rounded-full transition-all duration-200 ${
                    i === page
                      ? "w-6 h-2 bg-blue-600"
                      : "w-2 h-2 bg-line-200 hover:bg-ink-300"
                  }`}
      ...
      {/* ================= BLOG ================= */}
      <section className="max-w-[1200px] mx-auto px-4 sm:px-6 py-10">
        <h2 className="text-3xl font-bold text-ink-900 mb-8">Blog dan Artikel</h2>
            ...
            >
              <div
                className={`h-44 bg-gradient-to-br ${
                  article.coverGradient ?? "from-blue-600 to-blue-400"
